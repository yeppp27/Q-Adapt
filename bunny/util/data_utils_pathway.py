...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            #orin_image = transform(orin_image)
        ...
        if prompt is not None:
            ...
            data_dict['orin_image'] = orin_image
        if 'prompt' in self.list_data_dict[i]:
            data_dict['prompt'] = [self.list_data_dict[i]['prompt']]
        if 'pathway' in self.list_data_dict[i]:
            data_dict['prompt'] = [self.list_data_dict[i]['pathway']]
            #print('pathway:',data_dict['prompt'])
        return data_dict
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                ...
                batch['images'] = images
        if 'orin_image' in instances[0]:
            orin_image  = [instance['orin_image'] for instance in instances]
            batch['orin_image'] = orin_image
